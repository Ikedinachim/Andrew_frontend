...
const MCQPage = () => {
  ...
  const { quizSubmitData, quizSubmitStatus, quizSubmitError } = useSelector(
    (state) => state.quizSubmit
  )
  const { reportData, reportStatus, reportError } = useSelector(
    (state) => state.report
  )
    ...
    try {
      ...
      await dispatch(
        submitQuiz({
          ...
          body: { userAnswers: selectedAnswers },
        })
      )
      // setTimeout(() => {
  ...
  if (quizStatus == "idle" || quizStatus == "loading") {
    return <LoadingPage content="Generating Quiz" />
  }
  ...
  if (quizSubmitStatus == "success") {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(getQuizReport(window.localStorage.getItem("quiz_id")))
        navigate(
          `/dashboard/quiz-performance-report/${window.localStorage.getItem(
            "quiz_id"
          )}`
        )
      }
    ...
    <div className="h-[100vh] flex flex-col items-center ">
      <div className="w-[1011px] bg-gray-200 rounded-full h-2.5 mr-2 mt-13 ">
        <div
          ...
          className={`bg-[#040BC5] h-2.5 rounded-full`}
        ></div>
      </div>
      <h2 className="font-semibold text-[32px] text-[#333333] w-[808px] mt-[103px] mb-[90px] text-center mx-auto">
        {quizData.data.questions[current_index].questionText}
      </h2>
        ...
        {quizData.data.questions[current_index].options.map((answer, index) => {
          return (
            <MCQItem
              ...
              content={answer}
              correct={
                quizData.data.questions[current_index].correctAnswer === answer
              }
      ...
      <div className=" w-full h-[136px] shadow-xl flex px-6 flex-row items-center justify-between mt-2">
        ...
        <button
          disabled={!isSelected}
          className="bg-[#040BC5] size-fit text-white px-6 py-[12px] w-[167px] rounded-[8px]"
